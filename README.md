...
| 🗑️ **删除对话** | 按 `d` 删除对话，支持多选，删除后列表即时刷新 |
...
❀ 欢迎回来，admin ❀
  ...
  当前模型: ecnu-max    d) 删除对话记录  m) 查看更早历史  s) 切换模型    n) 更改昵称
...
- 北京时间 **09:00-12:00** 和 **14:00-18:00**（DeepSeek 高峰时段）：亮红色
- 其他时间：绿色
- 这是为了让用户一眼看出当前是否为 API 高峰时段
...
不能。删除是永久性的（与 `hermes sessions delete` 行为一致），删除前有确认提示。
